# Build Version: v10 - Core Active Groq Models Only
import os
os.environ["HOME"] = "/tmp"
...
<body class="bg-gray-50 font-sans text-gray-800">
    ...
    <main class="max-w-6xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        <section class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-[650px]">
            <div class="p-4 border-b border-gray-100 bg-gray-50 rounded-t-xl flex items-center justify-between">
                <h2 class="font-semibold text-gray-700 flex items-center gap-2">💬 Live AI Assistant (v10)</h2>
    ...
    <script>
        async function sendMessage() {
            ...
            chatBox.innerHTML += `<div id="${loadingId}" class="flex items-start"><div class="bg-gray-200 text-gray-600 rounded-2xl rounded-tl-none px-4 py-3 text-sm italic">Connecting to AI model...</div></div>`;
            chatBox.scrollTop = chatBox.scrollHeight;
            try {
...
def chat_with_ai(query: ChatQuery):
    ...
    # 100% Guaranteed Active Groq Models
    models_to_try = [
